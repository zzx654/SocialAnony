...
class TagFragment: Fragment(R.layout.fragment_tag) {
    ...
    override fun onCreateView(
        ...
        popularAdapter.setOnTagClickListener { tagResult ->
            findNavController().navigate(TagFragmentDirections.actionGlobalTagPostsFragment(tagResult.tagname))
        }
    ...
    private fun subscribeToObserver()
    {
        vmTag.toggleTagResponse.observe(viewLifecycleOwner,Event.EventObserver(
            ...
            if(it.isLiked==0)
            {
                favoriteAdapter.tags+=TagResult(it.tagname,null,null)//즐겨찾기목록에 추가
            }
            else{
                favoriteAdapter.tags-=TagResult(it.tagname,null,null)
                //즐겨찾기에서 삭제
            }
            ...
            for(i in popularAdapter.tags.indices)
            {//인기태그목록 순회
                if(it.tagname==popularAdapter.tags[i].tagname)
                {//인기태그목록의 태그명과 응답받은 태그명이ㅅ 같으면
